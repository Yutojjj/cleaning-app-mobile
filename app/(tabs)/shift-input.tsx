...
export default function ShiftInputScreen() {
  ...
  const [storeRequirements, setStoreRequirements] = useState<Record<string, number>>({});
      ...
      if (currentUser) {
        ...
        try {
          ...
          if (userDoc.exists()) {
            ...
            const autoSnap = await getDoc(doc(db, 'settings', 'autoShiftConfig'));
            if (autoSnap.exists()) {
              setStoreRequirements(autoSnap.data().storeRequirements || {});
            }
          }
        } catch {}
        fetchEvents();
  ...
  const isUnderstaffed = (dateKey: string): boolean => {
    for (const site of sitesList) {
      if (site === '他') continue;
      const required = storeRequirements[site] || 0;
      if (required === 0) continue;
      const assigned = mockStaff[site]?.filter(s => isAssigned(s.shifts[dateKey])).length || 0;
  ...
  const autoFillMonth = async () => {
    Alert.alert(
      '自動入力確認',
      `${currentMonth.getFullYear()}年${currentMonth.getMonth() + 1}月のシフトを自動入力します。よろしいですか？`,
      [
          ...
          text: '実行', onPress: async () => {
            try {
              ...
              for (let day = 1; day <= daysInMonth; day++) {
                ...
                const availableStaff = allStaffDB.filter(s => hasHope(s.shifts?.[dateKey]));
                ...
                for (const site of sitesList) {
                  ...
                  eligible.sort((a: any, b: any) => (leaderPriorityMap[a.id] ?? 9999) - (leaderPriorityMap[b.id] ?? 9999));

                  for (const staff of eligible.slice(0, required)) {
                    if (!allUpdates[staff.id]) allUpdates[staff.id] = {};
                    if (!allUpdates[staff.id][dateKey]) allUpdates[staff.id][dateKey] = {};
                    allUpdates[staff.id][dateKey][site] = '出勤';

                    if (!newMock[site]) newMock[site] = [];
                    const idx = newMock[site].findIndex((s: Staff) => s.id === staff.id);
                    if (idx >= 0) {
                      newMock[site][idx] = { ...newMock[site][idx], shifts: { ...newMock[site][idx].shifts, [dateKey]: '出勤' } };
                    } else {
                      newMock[site].push({ id: staff.id, name: staff.name, role: staff.role, shifts: { [dateKey]: '出勤' } });
                    }
                  }
                }
              }

              for (const [userId, dateUpdates] of Object.entries(allUpdates)) {
                const userRef = doc(db, 'users', userId);
                const userDoc = await getDoc(userRef);
                if (userDoc.exists()) {
                  const currentShifts = userDoc.data().shifts || {};
                  for (const [dateKey, siteShifts] of Object.entries(dateUpdates)) {
                    const dayShift = currentShifts[dateKey] || {};
                    const newDayShift = typeof dayShift === 'string' ? {} : { ...dayShift };
                    Object.assign(newDayShift, siteShifts);
                    currentShifts[dateKey] = newDayShift;
                  }
                  await updateDoc(userRef, { shifts: currentShifts });
                }
              }
              ...
              setMockStaff(newMock);
              setStoreRequirements(autoConfig.storeRequirements || {});
              Alert.alert('完了', `${year}年${month + 1}月のシフトを自動入力しました`);
            } catch {
  ...
  const getWeekRowHeight = (week: (number | null)[]): number => {
    if (!showNames) return Math.round(screenHeight / 10);
    const year = currentMonth.getFullYear();
    const month = currentMonth.getMonth();
    let maxNames = 0;
    for (const day of week) {
      ...
      let count = 0;
      for (const site of sitesList) {
        count += mockStaff[site]?.filter(s => isAssigned(s.shifts[dk])).length || 0;
      }
      maxNames = Math.max(maxNames, count);
    }
    return Math.max(55, 28 + maxNames * 13);
  };
      ...
      <View style={styles.header}>
        ...
        {isAdmin && (
          <View style={{ flexDirection: 'row', gap: 14, alignItems: 'center' }}>
            <TouchableOpacity onPress={() => setShowNames(v => !v)}>
              <Ionicons name={showNames ? 'list-outline' : 'people-outline'} size={24} color="#B8860B" />
            </TouchableOpacity>
            <TouchableOpacity onPress={autoFillMonth}>
              <Ionicons name="flash" size={24} color="#B8860B" />
            </TouchableOpacity>
          </View>
        )}
      </View>
        ...
        <View style={localStyles.calendarCard}>
          ...
          {isAdmin ? (
            getWeeks().map((week, wi) => {
              const rowH = getWeekRowHeight(week);
              const year = currentMonth.getFullYear();
              const month = currentMonth.getMonth();
              return (
                <View key={wi} style={{ flexDirection: 'row' }}>
                  {week.map((day, di) => {
                    ...
                    const understaffed = day ? isUnderstaffed(dateKey) : false;
                    ...
                    if (day) {
                      ...
                      for (const site of sitesList) {
                        const cnt = mockStaff[site]?.filter(s => isAssigned(s.shifts[dateKey])).length || 0;
                        if (cnt > 0) assignedCounts[site] = cnt;
                        mockStaff[site]?.forEach(staff => {
                          if (staff.shifts[dateKey] && staff.shifts[dateKey] !== '休み') hopeIds.add(staff.id);
                        });
                      }
                      hopeCount = hopeIds.size;
                    }
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        style={[
                          localStyles.dayCell,
                          { height: rowH },
                          understaffed && { backgroundColor: '#FFF0F0' },
                        ]}
                        onPress={() => day && handleDayPress(dateKey)}
                        disabled={!day}
                      >
                        <View style={{ flexDirection: 'row', alignItems: 'baseline', width: '100%', paddingLeft: 2 }}>
                          <Text style={localStyles.dayNum}>{day || ''}</Text>
                          {day && hopeCount > 0 && <Text style={localStyles.miniTotalCount}>{hopeCount}名</Text>}
                        </View>
                        {eventTitle && <View style={localStyles.eventBadge}><Text style={localStyles.eventText} numberOfLines={1}>📌 {eventTitle}</Text></View>}
                        {understaffed && day && (
                          <View style={localStyles.understaffBadge}>
                            <Text style={localStyles.understaffText}>不足</Text>
                          </View>
                        )}
                          ...
                          <View style={{ width: '100%', marginTop: 1 }}>
                            {sitesList.map(site => {
                              ...
                              return (
                                <View key={site}>
                                  {siteStaff.map(s => (
                                    <Text key={s.id} style={{ fontSize: 7.5, color, fontWeight: 'bold', lineHeight: 12 }} numberOfLines={1}>
                                      {getSiteAbbrev(site)}:{s.name.length > 4 ? s.name.slice(0, 4) : s.name}
                                    </Text>
                                  ))}
                                </View>
                              );
            ...
            <View style={localStyles.daysGrid}>
              {getWeeks().flat().map((day, i) => {
                ...
                return (
                  <TouchableOpacity
                    key={i}
                    style={[localStyles.dayCell, { height: Math.round(screenHeight / 10) }, hasShift && localStyles.dayCellActive, eventTitle && { backgroundColor: '#FFFBEB' }]}
                    onPress={() => day && handleDayPress(dateKey)}
                  ...
                  >
                    ...
                    {eventTitle && <View style={localStyles.eventBadge}><Text style={localStyles.eventText} numberOfLines={1}>📌 {eventTitle}</Text></View>}
                    {hasShift && <Text style={{ color: '#10B981', fontSize: 22, fontWeight: 'bold', marginTop: 3 }}>〇</Text>}
                  </TouchableOpacity>
                );
              })}
            </View>
      ...
      >
        <SafeAreaView style={styles.safeArea}>
          <View style={styles.header}>
            ...
            <Text style={[styles.headerTitle, { color: '#B8860B', fontSize: 16 }]}>
                ...
                return (
                  <View key={site} style={{ marginBottom: 35 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10 }}>
                      <Text style={[localStyles.detailSiteTitle, { color, marginBottom: 0 }]}>{site.toUpperCase()}</Text>
                      {required > 0 && site !== '他' && (
                        <Text style={{ marginLeft: 10, fontSize: 12, color: workingStaff.length >= required ? '#10B981' : '#EF4444', fontWeight: 'bold' }}>
                          {workingStaff.length}/{required}名
                        </Text>
                      )}
                    </View>
          ...
          {modalView === 'addStaff' && (() => {
            ...
            const assignedStaff = mockStaff[targetSiteForAdd!]?.filter(s => isAssigned(s.shifts[selectedDate!])) || [];
                    ...
                    <View style={{ marginBottom: 20 }}>
                      ...
                      {assignedStaff.map(staff => (
                        <View key={staff.id} style={[localStyles.addStaffItem, { borderColor: targetColor, borderWidth: 1 }]}>
                          ...
                          <TouchableOpacity style={localStyles.detailDeleteBtn} onPress={() => deleteStaffFromDate(staff.id, targetSiteForAdd!)}>
                            <Ionicons name="trash-outline" size={20} color="#ef4444" />
                          </TouchableOpacity>
                        </View>
                      ))}
...
const localStyles = StyleSheet.create({
  ...
  dayCell: { width: '14.28%', borderBottomWidth: 0.5, borderRightWidth: 0.5, borderColor: '#F1F5F9', padding: 2, alignItems: 'center', overflow: 'hidden' },
  dayCellActive: { backgroundColor: '#F0FDF4' },
  dayNum: { fontSize: 14, color: '#475569', marginTop: 2, fontWeight: 'bold' },
  miniTotalCount: { fontSize: 9, color: '#B8860B', fontWeight: 'bold', marginLeft: 1 },
  dayNumberActive: { color: '#B8860B', fontWeight: 'bold' },
  siteCountText: { fontSize: 8, fontWeight: 'bold', textAlign: 'center', marginTop: 1 },
  eventBadge: { marginTop: 1, backgroundColor: '#FFFBEB', padding: 1, borderRadius: 3, width: '100%', borderWidth: 0.5, borderColor: '#F59E0B' },
  eventText: { color: '#92400E', fontSize: 7, fontWeight: 'bold', textAlign: 'center' },
  understaffBadge: { backgroundColor: '#FEE2E2', paddingHorizontal: 3, paddingVertical: 1, borderRadius: 3, marginTop: 1 },
  understaffText: { fontSize: 7, color: '#DC2626', fontWeight: 'bold' },
